...
ElectionsService.countElectionByKontestanId = (kontestan_id, bearer_id, tps_code) => __awaiter(void 0, void 0, void 0, function* () {
    let query = election_1.Elections.query()
    ...
    if (tps_code)
        query.andWhere("elections.tps_code", tps_code);
    return query;
});
...
ElectionsService.getCountElectionSummaryByKonId = (bearer_id, kontestan_id, tps_code) => {
    let query = election_summary_1.ElectionSummary.query()
        .count("election_summary.id as summary")
        .groupBy("election_summary.kontestan_id")
        ...
        .andWhere("kontestan:users.bearer_id", bearer_id);
    if (tps_code)
        query.andWhere("election_summary.tps_code", tps_code);
    return query;
};
